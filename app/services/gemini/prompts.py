...
위 종목들을 분석하여 기준을 충족하는 종목을 최대 {pick_count}개 선정하고 근거를 서술하세요.

【ai_probability 정의 — 엄격히 준수】
ai_probability는 "보유기간 {hold_days}일 이내에, 손절라인(-{stop_loss_pct}%)을 단 한 번도 터치하지 않고 목표수익률(+{target_pct}%)에 먼저 도달할 확률"이다.
- 결국 오르더라도 중간에 손절선을 터치할 위험이 있으면 확률을 낮게 산출할 것
- 아무리 상승 기대가 크더라도 변동성이 커서 하락 경로가 손절선에 닿을 수 있으면 낮게 평가할 것

【종목 평가 우선순위】
1. 하방 지지선 (MA60, 52주 저점 등)이 현재가 기준 손절선(-{stop_loss_pct}%) 위에 버텨주고 있는가
2. 기관·외국인 수급이 하락 시 받쳐주는 구조인가 (순매수 지속 여부)
3. 최근 RSI가 30~55 구간인가 (이미 충분히 눌려 추가 낙폭 여지가 적은 종목 우선)
4. 위 조건을 모두 충족하는 종목이 없으면 억지로 선정하지 말 것

【작성 규칙】
- 종목을 언급할 때 반드시 코드와 이름을 함께 쓰세요. 예: 330860(네패스아크)
- 각 종목의 선정 근거, 역사적 유사 사례, 리스크를 포함하세요.
- JSON 없이 자연스러운 분석 텍스트로 작성하세요.
- 매크로 환경이 불리하거나 min_probability를 충족하는 종목이 없으면 선정 종목 없음으로 결론 내려도 된다. {pick_count}개를 채우기 위해 억지로 선정하지 말 것.
"""
