...
- 최근 주가 변동의 동인 (필수 확인): {price_move_note} — 이 변동의 동인으로 지목되는 뉴스/공시를 반드시 검색해 확인할 것. 확인되지 않으면 논거에 "동인 확인 불가"로 명시하고, 관련 없는 기사를 억지로 연결하지 말 것.
...
4. 무효화_조건은 반드시 관측 가능한 신호로 쓸 것 (falsifiable) — 나쁜 예: "실적이 나빠지면" / 좋은 예: "다음 분기 단일분기 영업이익률이 X% 아래로 내려가면", "외국인 누적 순매도가 N거래일 이상 지속되면", "원/달러가 X원을 하회 반전하면".
...
def run_analysis(db, user_id: uuid.UUID, stock_code: str, stock_name: str,
    ...
    if _count_recent_news(result.get("뉴스_출처"), analysis_date) == 0:
        ...
        if r2.get("무효화_조건"):  # 재요청 응답이 핵심 필드를 갖췄을 때만 교체
